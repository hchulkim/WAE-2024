...
k_grid = [1+(0.25/250)*(i-251) for i in 1:501] # Discrete grid for capital
...
for i in 1:iterations
    ...
    c_index[i] = (1/(0.3*0.6))* k_initial^(0.3) - closest_value
...
savefig("figures/plot1-8_k-value.png")  # save the value function plot

plot(1:iterations, c_values, xlabel="Iteration", ylabel="C Value", title="first 100 elements", marker=:circle)
savefig("figures/plot1-8_c-value.png")  # save the value function plot
...
XLSX.openxlsx("tables/values_output_p1-8.xlsx", mode="w") do xf
    sheet = xf[1]
    # Write headers
    sheet["A1"] = "Iteration"
    sheet["B1"] = "K Values"
